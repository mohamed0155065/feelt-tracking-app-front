...
export const UpdateDriverModal: React.FC<UpdateDriverModalProps> = ({ isOpen, onClose,driver }) => {

  const { register, handleSubmit, onSubmit, errors, isPending,reset } = useUpdateDriver(driver.id,{
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-zinc-950/60 backdrop-blur-sm p-4 animate-in fade-in duration-200">
      <div className="bg-white w-full max-w-md rounded-3xl p-6 shadow-2xl relative border border-slate-100 animate-in zoom-in-95 duration-200 text-start font-cairo">
        ...
        <h3 className="text-base font-bold text-slate-900">تعديل بيانات السائق </h3>
        <p className="text-[11px] text-slate-400 mt-0.5">أدخل بيانات السائق الجديده</p>
