...
import { Truck, Square } from 'lucide-react';

/** الإحداثيات الابتدائية الافتراضية للسائق */
const INITIAL_COORDS = { lat: 24.7136, lng: 46.6753 };

/**
 * مكون واجهة محاكاة تطبيق السائق على الجوال
 * يتيح بدء/إيقاف التتبع اللحظي وبث الإحداثيات بالخلفية
 */
export const DriverInterface: React.FC = () => {
  const [isTracking, setIsTracking] = useState(false);
  const [coords, setCoords] = useState(INITIAL_COORDS);

  // مرجع لحفظ معرّف الـ Interval لضمان تنظيفه وإلغائه بدقة عند الإيقاف
  const intervalRef = useRef<NodeJS.Timeout | null>(null);

  /**
   * دالة التبديل بين تشغيل وإيقاف التتبع مع إعادة الضبط الفوري
   */
  const handleToggleTracking = () => {
    if (isTracking) {
      // عند الإيقاف: إلغاء الـ Interval وتصفير الإحداثيات
  ...
  // إدارة بث الإحداثيات والتنظيف اللحظي
  useEffect(() => {
    if (isTracking) {
  ...
  return (
    <div className="w-full max-w-sm bg-[#0B132B] border border-slate-800 rounded-[40px] p-6 shadow-2xl text-center flex flex-col justify-between min-h-[580px] animate-in fade-in zoom-in-95 duration-500 relative overflow-hidden">
      ...
      {/* شريط الساعة وحالة الاتصال العلوي */}
        ...
        <div className="flex items-center gap-1.5 bg-slate-900/60 px-2 py-1 rounded-full border border-slate-800">
          <span
            className={`w-2 h-2 rounded-full ${
              isTracking ? 'bg-emerald-500 animate-ping' : 'bg-red-500'
            }`}
          />
      ...
      <div className="flex flex-col items-center justify-center my-auto relative z-10">
        <div className="relative flex items-center justify-center">
          ...
          <button
            ...
            className={`w-36 h-36 rounded-full font-bold text-sm transition-all duration-300 shadow-2xl flex flex-col items-center justify-center gap-2 active:scale-90 border select-none cursor-pointer ${
              isTracking
